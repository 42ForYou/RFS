...
const MemoComponent = 6; // 메모컴포넌트에서 사용되는 Fiber로 만약 simpleFunctionComponent이고 compare가 없다면
//simpleMemoComponent로 처리되고 아니면 memoComponent로 처리된다.

const SimpleMemoComponent = 7; // 함수형 컴포넌트를 위한 메모 컴포넌트
const IndeterminateComponent = 8; // 원래 리액트에선 함수형인지 아닌지에 따라서 처리를 분리하기 위해서 되어 있는데,
//현 RFs에서는 함수형 컴포넌트의 마운트를 위한 타입으로 사용된다.
export const ForwardRef = 9; //ForwardRef --> 자식에게 ref를 전달하기 위한 타입
